...
\title{An adaptive parallel algorithm for Vector Error Correction models applied
...
even though individually they are not. Furthermore, the number of cointegration
vectors reflects how many of these linear combinations exist.  Some models,
...
VECM introduces this long-run relationship among a set of cointegrated
variables as an error correction term. VECM is a special case of the vector
autoregressive model (VAR) model \cite{sims1980}. VAR model expresses future
values as a linear combination of variables past values.  However, the VAR
model cannot be used with non-stationary variables. VECM is a linear model but
in terms of variable differences. If cointegration exists, variable differences
...
Based on this fact, we proposed in this paper a new forecast method which
determines adaptively the number of observations and lags of VECM based on
cointegration which we called Adaptively Vector Error Correction (AVECM).  Since
cointegration often improves forecasting, model parameters will be those which
maximize cointegration relations in the past.  This search is computationally
expensive because to find cointegration vectors we used the Johansen
method~\cite{johansen1995}, which is of order $O(n^3)$. Therefore, our proposal
is to parallelize this search in order to get new parameters at every iteration.
...
This paper is organized as follows: section~\ref{sec:background} presents the
...
From the previous section \ref{sec:background} we know that when $r=0$, 
there is no cointegration and $r=l$ reveals that no process is I(1) but,
...
\begin{figure}[!h]
  %\vspace{-0.8cm}
  \centering
  \includegraphics[width=0.45\textwidth]{histCointVectors}
...
\begin{figure}[!h]
  %\vspace{-0.8cm}
  \centering
  \includegraphics[width=0.45\textwidth]{histCointVectorsp5}
  \caption{Histogram of the number of cointegration vectors using $p=5$. 
   Despite the chande in the distribution of cointegration vectors, 
...
\begin{figure}[!h]
  %\vspace{-0.8cm}
  \centering
  \includegraphics[width=0.48\textwidth]{MAPEvsCoint-offset21600-p-2-freq-10s}
...
step. 
The criterion for choosing $L$ and $p$ is to maximize the percentage of
cointegration $PC$ considering a number of past iterations $it$.

Algorithm ~\ref{alg:AVECM} summarizes our proposal. 
The function {\bf get\_best\_params} makes a grid search on the two vector
lists $Ls$ and $ps$ and returns the parameters $L$ and $p$ which maximize
...
\subsection{Parallel implementation} \label{sec:paralell}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.5\textwidth]{extimes}
...
transferring data and synchronizing process is about two seconds independently
...
maximizing the percentage of cointegration of the last samples or iterations.
...
parameters of $L$ and $p$ maximizing the percentage of cointegration.
...
\bibitem{arestis2001}
Philip Arestis, Panicos~O Demetriades, and Kul~B Luintel.
\newblock Financial development and economic growth: the role of stock markets.
\newblock {\em Journal of Money, Credit and Banking}, pages 16--41, 2001.

\bibitem{armstrong1992}
J~Scott Armstrong and Fred Collopy.
\newblock Error measures for generalizing about forecasting methods: Empirical
  comparisons.
\newblock {\em International journal of forecasting}, 8:69--80, 1992.

\bibitem{banerjee1993}
A.~Banerjee.
\newblock {\em Co-integration, Error Correction, and the Econometric Analysis
  of Non-stationary Data}.
\newblock Advanced texts in econometrics. Oxford University Press, 1993.

\bibitem{Dukascopy2014}
Dukascopy.
\newblock Dukascopy historical data feed, 2014.

\bibitem{duy1998}
Timothy~A Duy and Mark~A Thoma.
\newblock Modeling and forecasting cointegrated variables: some practical
  experience.
\newblock {\em Journal of Economics and Business}, 50(3):291--307, 1998.

\bibitem{engle87}
Robert~F. Engle and C.~W.~J. Granger.
\newblock {Co-Integration and Error Correction: Representation, Estimation, and
  Testing}.
\newblock {\em Econometrica}, 55(2):251--276, March 1987.

\bibitem{engle2004}
Robert~F Engle and Andrew~J Patton.
\newblock Impacts of trades in an error-correction model of quote prices.
\newblock {\em Journal of Financial Markets}, 7(1):1--25, 2004.

\bibitem{golub1980}
Gene~H Golub and Charles~F Van~Loan.
\newblock An analysis of the total least squares problem.
\newblock {\em SIAM Journal on Numerical Analysis}, 17(6):883--893, 1980.

\bibitem{gregoryETal1996}
Allan~W Gregory, James~M Nason, and David~G Watt.
\newblock Testing for structural breaks in cointegrated relationships.
\newblock {\em Journal of Econometrics}, 71(1):321--341, 1996.

\bibitem{herlemont2003}
Daniel Herlemont.
\newblock Pairs trading, convergence trading, cointegration.
\newblock {\em YATS Finances \& Techonologies}, 5, 2003.
...
Soren Johansen.
\newblock {\em Likelihood-Based Inference in Cointegrated Vector Autoregressive
  Models}.
\newblock Oxford University Press, 1995.

\bibitem{maysami2000}
Ramin~Cooper Maysami and Tiong~Sim Koh.
\newblock A vector error correction model of the singapore stock market.
\newblock {\em International Review of Economics \& Finance}, 9(1):79--96,
  2000.

\bibitem{mukherjee1995}
Tarun~K Mukherjee and Atsuyuki Naka.
\newblock Dynamic relations between macroeconomic variables and the japanese
  stock market: an application of a vector error correction model.
\newblock {\em Journal of Financial Research}, 18(2):223--37, 1995.
...
\newblock Estimation of vector error correction models with mixed-frequency
  data.
\newblock {\em Journal of Time Series Analysis}, 34(2):194--205, 2013.

\bibitem{sims1980}
Christopher~A Sims.
\newblock Macroeconomics and reality.
\newblock {\em Econometrica: Journal of the Econometric Society}, pages 1--48,
  1980.

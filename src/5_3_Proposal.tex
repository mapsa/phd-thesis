...
ordinary least squares method (OLS). Since these parameters could be very
sensitive to the data, this proposal explores the use of ridge regression (RR)
and the agregating algorithm for regression (AAR) which could lead to a better
generalization capability. In the previous chapter OVECM was presented, which is
an online version of the traditional VECM (Vector Error Correction) that uses
OLS to get the model parameters. In this chapter a variation of OVECM is
presented. RR and AAR will be used instead of OLS to get the model parameters in
an online way.

\section{The problem}

Traditionally VECM parameters are obtained using ordinary least squares

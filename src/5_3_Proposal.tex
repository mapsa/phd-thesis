...
following matrices which considers only the last $L$ instances:
...
The RR solution $\mathbf{X}(t)$ using the sliding window matrices defined above
is:

\begin{equation}
\label{eq:optsolSLAAR}
%\mathbf{X}(t)&=&()^{-1}{\bf A}(t)^\top{\bf B}(t) \\

...
In figure~\ref{fig:returndist} is shown the SPY returns distribution based
...
\item If $f(t) = \displaystyle\frac{1}{k+\alpha \beta ^t }$ we have a logistic curve
\end{itemize}
...
by differencing the time series once. The disadvantage of differencing is that
...
\eqref{eq:rsquared} becomes very large because a large weight is placed on
...
Figure~\ref{fig:spurious1} shows the time series $x_{1,t}$ and $x_{2,t}$ and
...
(shown in equation~\eqref{eq:johansentrace}) and the maximum-eigenvalue statistic
(shown in equation~\eqref{eq:johansenmax}):
...
\noindent where $r$ is the number of cointegration vectors, $T$ is the number of observations and
...
If we have a two-dimensional process $\mathbf{y}_t$, $t=1,\dots,T$ by:
...
Figure~\ref{fig:cointex} shows the three time series examples.
...
\eqref{fig:cointvectors5}  shows the linear combinations obtained using
...
The $\text{ADF}_t$ statistic is based on the least squares estimates of equation~\eqref{eq:adftest}.
...
\subsubsection{Realised volatility} is also known as historic volatility and
...
\begin{eqnarray}
r_{t}&=&100(\ln(p_{t,N}) - \ln(p_{t-1,N})) \label{eq:ret1} \\
\bar{\rho}(t) &=& \sum_{t=1}^T r_{t}^2  \label{eq:ret2} \, .
\end{eqnarray}
\noindent where equation~\eqref{eq:ret1} represents overnight 
returns and the volatility as 
equation~\eqref{eq:ret2}, where $p_{t,N}$ is the last intraday 
...
as the daily squared returns as is shown in equation~\eqref{eq:ret2}.
...
shown in equation~\eqref{eq:rv}, is less noisy and doesn't lead to choosing an
inferior model.   
\subsubsection{Implied volatility} not only can be extracted from returns
...
equation~\eqref{eq:stockprice} is:
...
For trading strategies, the interest is centred in forecasting realised
...
The realised volatility constructed from high frequency intraday returns gave
...
There are several multivariate models widely used such as the multivariate ARCH model \cite{bollerslev1990modelling} and variations like the DCC-GARCH \cite{engle2002dynamic}, the BEKK model \cite{10.2307/3532933} and the VAR model  \cite{sims1980}. 
The vector autoregressive VAR($p$) model derives the vector correction model VECM, which studies the cointegration between time series and also is one of the most easy
to use and flexible models for the analysis of multivariate time
series. 

The VAR model is a natural extension of the univariate autoregressive (AR) model.
...
Notice that, regarding our notation of the cointegration section
\eqref{sec:cointegration}, we have here 
$\mathbf{y}_t^0 = \mathbf{y}_t$,
$\mathbf{y}_t^\nu = \mathbf{y}_{t-\nu}$ and
the $i$-th component of the vector time series $\mathbf{y}_t^\nu$
is the scalar time series $y_{i,t}^\nu$, where $\nu=1,\dots,p$ and
$i=1,\dots,l$.
Transposing each equation of the system~\eqref{eq:var} we can write
...
&= \begin{bmatrix}
   ...
   \end{bmatrix}_{(N-p)\times l}
\end{alignat*}
Taking into account the error term $\mathbf{E}$, equation~\eqref{eq:simplevareq}
...
VECM  is obtained re-writing equation~\eqref{eq:var} in terms of the new
...
$\boldsymbol{\Phi}_i$ of the model VAR shown in equation~\eqref{eq:var}, are:
\begin{align*}
\boldsymbol{\Phi}_i^* 
...
If cointegration exists, then equation~\eqref{eq:vec} can be written
as follows:
...
Transposing each equation of the system (\eqref{eq:vecfull}) we can write
...
Taking into account the error term $\mathbf{E}$, equation~\eqref{eq:simpleveceq}
can be solved with respect to $\mathbf{X}$ using the ordinary least
squares estimation (see section~\ref{sec:OLS}).
...
$\boldsymbol{\Omega}=\boldsymbol{\alpha\beta}^\top$ on equation~\eqref{eq:vec} called trace and maximal
...
get the cointegration matrix $\boldsymbol{\beta}^\top$.  Critical values for both
...
When $\mathbf{A}$ is singular, solution to equation~\eqref{eq:simpleveceq} is given
...
always singular and equation~(\eqref{eq:pseudoinv}) cannot be used.
...
More details can be found in section \eqref{app:pscsvd}.

...
A learning algorithm $\mathcal{A}$ takes as input a data set $S \in \mathcal{S}$ and outputs a function $f$:
...
\noindent where $\mathcal{H}$ is called the {\em hypothesis space}, is the space of functions that the algorithm is allowed to search. The selection of $f_S$ is based on the minimization if the expected risk of the loss function $V(f(x),y): \R\times\R\to\R$.

...
In this thesis we will emphasise the supervised learning problem since it is the most common way of modelling financial problems.
...
$$
S= \{(x_1,y_1),\,(x_2,y_2),\dots\,,\,(x_n,y_n)\} \in \mathcal{S}, (x_i,y_i) \in X\times Y \quad \forall i = 1..n
$$

\noindent where $\mathcal{S}$ is the space of instances, $X \subseteq \mathbb{R}^m$ and $Y = \mathbb{R}$ in case of regression problems and $Y = \{1,-1\}$  if it is a classification problem. 
...
\noindent where $\mathcal{H}$ is called the {\em hypothesis space}, is the space of functions that the algorithm is allowed to search. The selection of $f_S$ is based on a loss function $V(f(x),y)$ the expected risk, which has to be minimised.

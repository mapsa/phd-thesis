...
A learning algorithm $\mathcal{A}$ takes as input a data set $S \in \mathscr{T}$ and selects from $\mathcal{H}$ a function $f$ such that $f(x)\approx y $ in a predictive way:
...
I[f] = E[V(f(x),y)] = \int_{X \times Y} V(f(x),y) \; dp(x,y)
\end{equation}

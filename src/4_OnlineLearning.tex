...
given by equation~(\ref{eq:prederror}). Despite OLS has zero bias,
...
model. Moreover, it could happen that we won't have enought time to process the
...
preferable computationally if the existing hypothesis or other artefact of
...
Competitive analysis was designed for analysing the performance of an online algorithm compared with its optimal offline algorithm. An optimal offline algorithm can view the sequences of requests in advance. The effectiveness of an online algorithm \cite{} may be measured by its competitive ratio which is the worst-case ratio of the online algorithm and the the optimal offline algorithm.
